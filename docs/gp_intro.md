...
* A GP model with the RBF kernel on context dimensions, but a linear kernel on intensity dimensions, additively composed. 
...
## GP model with RBF-Linear additive kernel
An earlier and more restrictive approach to constraining psychometic field intensity dimensions comes from [Song et al. 2017](https://link.springer.com/article/10.3758/s13414-017-1460-0), who decompose the GP kernel additively into an RBF kernel over context dimensions, and a linear kernel over the context dimension defined as $\kappa_i(x_i, x_i') = x_i x_i^{T}$. This model assumes that the psychometric function has the same linear appearance over the whole space, and is shifted nonlinearly based on context. 

**Usage Advice**. This model is provided for benchmarking relative to prior work, but we currently do not recommend it, as [Keeley et al. 2023](https://arxiv.org/abs/2302.01187) showed that it under-performs the semi-parametric models in the small-data regime, and the full RBF models in the larger-data regime. 

## Pairwise models
Often in both psychophysics and preference learning participants make binary responses based on pairs of stimuli (e.g. "is X preferred to Y", or "is X louder than Y"). For this setting, AEPsych provides a pairwise classification model which is a thin wrapper around the [pairwise preference GP](https://botorch.org/tutorials/preference_bo) in BoTorch. 

**Usage Advice**. This model is currently the only option for paired data in AEPsych. 
...
**Usage Advice** This is uncommon in psychophysics applications, but can be useful in a preference learning setting when users give scores on a 0-100 scale. 
...
**Usage Advice**: likert-scale data is common in user experience and other preference-oriented studies, but we believe based on the framing above that it has some promise in psychophysics as well. 

## Models for continuous data
For continuous data, we use a Gaussian likelihood with the identity link, i.e. $y \mid x \sim \mathcal{N}(\mathbf{f}(x)+ \sigma^2)$ with a learned variance $\sigma^2$. 
...
1. Monte Carlo (MC) acquisition functions. These acquisition functions can be evaluated by sampling from the model posterior. In the simplest case we sample from the latent perceptual field posterior, but we can also sample from other posteriors such as the response probability posterior, or response threshold posterior. The most common acquisition functions of this kind is BALD, or information maximization w.r.t. the response probability posterior (aka `BernoulliMCMutualInformation` in AEPsych), or BALV, or variance maximization (aka `MCPosteriorVariance` in AEPsych). [Keeley et al. 2023](https://arxiv.org/abs/2302.01187) additionally introduced `ThresholdBALV`, which is BALV defined over the threshold rather than probability posterior. Because they only require sampling from the posterior, they can be used with any model which admits sampling from the posterior. 
