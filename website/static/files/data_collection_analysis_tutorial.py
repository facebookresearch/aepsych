...
def show_gabor(theta, ax):
    ...
    M = np.array(
        [
            [gabor(i, j, theta) for j in range(-radius, radius)]
...
def run_trial(angle, trial_number):
    ...
    while ans not in ans_key:
        ...
        clear_output()
        ans = input(
            f"Trial #{trial_number}: Which side was angled? (Enter 'F' for left or 'J' for right)"
        )
...
replay(serv, exp_ids[-1], skip_computations=True)
...
plot_strat(
    ...
    ylabel="Probability of Selecting Target",
    target_level=0.75,
)
