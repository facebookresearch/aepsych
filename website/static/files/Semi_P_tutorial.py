...
xtrain = make_scaled_sobol(
    lb=lb, ub=ub, size=100
)  # using random sobol sampling here for data generation
...
### fit SemiP models
semip_model = SemiParametricGPModel(
    lb=lb,
    ub=ub,
    dim=2,
    stim_dim=1,
...
approx_model = HadamardSemiPModel(
    lb=lb,
    ub=ub,
    dim=2,
    stim_dim=1,
...
axs[0].imshow(
    norm.cdf(fgrid).reshape(30, 30).T,
    aspect="auto",
    origin="lower",
    vmin=0.5,
    vmax=1,
...
axs[2].imshow(
    approx_pred_mu.reshape(30, 30).T,
    aspect="auto",
    origin="lower",
    vmin=0.5,
    vmax=1,
