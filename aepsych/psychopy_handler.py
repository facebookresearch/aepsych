...
class AEPsychHandler(StairHandler):
    ...
    def __next__(self):
        ...
        if not self.finished:
            ...
            self.thisTrialN += 1
            if np.random.normal() < self.epsilon_prob:
                next_intensity = np.random.uniform(
                    low=self.server.strat.lb, high=self.server.strat.ub
                )
                next_intensity = self.server._tensor_to_config(next_intensity)
                self.intensities.append(next_intensity)
    ...
    def from_config(cls, config_message):
        ...
        names = config.str_to_list(config.get("experiment", "parnames"), element_type=str)
