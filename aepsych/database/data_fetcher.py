...
class DataFetcher(ConfigurableMixin):
    ...
    def __init__(self, exp_names: Optional[List[str]] = None, exp_desc: Optional[List[str]] = None, 
                 exp_ids: Optional[List[str]] = None, par_ids: Optional[List[str]] = None, 
                 ex_data: Optional[Dict[str, str]] = None, **kwargs):
    ...
    def _build_query(self) -> str:
        ...
        if self.experiment_names is not None:
            query_clauses.append(f"experiment_name in {_print_in_params(self.experiment_names)}")

        if self.experiment_ids is not None:
            query_clauses.append(f"experiment_id in {_print_in_params(self.experiment_ids)}")

        if self.experiment_desc is not None:
            query_clauses.append(f"experiment_description in {_print_in_params(self.experiment_desc)}")

        if self.participant_ids is not None:
            query_clauses.append(f"participant_id in {_print_in_params(self.participant_ids)}")

        if self.extra_metadata is not None:
            query_clauses.append(f"{_print_like_params('extra_metadata', self.extra_metadata)}")
    ...
    def _is_ex_metadata_match_valid(self, config: Config) -> bool:
        ...
        for key in potential_matches:
            if pickled_ex_data[key] == self.extra_metadata[key] or\
               (isinstance(self.extra_metadata[key], Iterable) and pickled_ex_data[key] in self.extra_metadata[key]):
                return True
...
def _print_warning(fetcher: DataFetcher, query_record: DBMasterTable, format: str):
    first_match = True
    match_str = ""
    if fetcher.experiment_names and query_record.experiment_name in fetcher.experiment_names:
        match_str += f"experiment_name = {query_record.experiment_name}"
        first_match = False

    if fetcher.experiment_desc and query_record.experiment_description in fetcher.experiment_desc:        
        if not first_match:
            match_str += " and "
    ...
    if fetcher.participant_ids and query_record.participant_id in fetcher.participant_ids:
        if not first_match:
            match_str += " and "
