...
def print_in_params(list_to_print: List[Any]) -> str:
    out_string = "("
...
def print_like_params(column_name: str, dict_to_print: Dict[str, str]) -> str:
    out_string = "("
...
class DataFetcher(ConfigurableMixin):
    ...
    def __init__(self, exp_names: List[str], exp_desc: List[str], exp_ids: List[str],
                 par_ids: List[str], ex_data: Dict[str, str], **kwargs):
        """Initialize the DataFetcher object.

        Args:
            exp_names (List[str]): A list of experiment names to use as a filter on warm start data.
            exp_desc (List[str]): A list of experiment descriptions to use as a filter on warm start data.
            exp_ids (List[str]): A list of experiment ids to use as a filter on warm start data.
            par_ids (List[str]): A list of participant ids to use as a filter on warm start data.
            ex_data (Dict[str, str]): A map of key-value pairs to use as a filter on warm start data.
        """
        self.experiment_names: List[str] = exp_names
        self.experiment_desc: List[str] = exp_desc
        self.experiment_ids: List[str] = exp_ids
        self.participant_ids: List[str] = par_ids
        self.extra_metadata: Dict[str, str] = ex_data
    ...
    def _has_search_criteria(self) -> bool:
        ...
        """
        return self.experiment_ids is not None or\
        self.experiment_names is not None or\
        self.experiment_desc is not None or\
        self.participant_ids is not None or\
        self.extra_metadata is not None
    
    def _build_query(self) -> str:
        query_clauses: List[str] = []
        if self.experiment_names is not None:
            query_clauses.append(f"experiment_name in {print_in_params(self.experiment_names)}")

        if self.experiment_ids is not None:
            query_clauses.append(f"experiment_id in {print_in_params(self.experiment_ids)}")

        if self.experiment_desc is not None:
            query_clauses.append(f"experiment_description in {print_in_params(self.experiment_desc)}")

        if self.participant_ids is not None:
            query_clauses.append(f"participant_id in {print_in_params(self.participant_ids)}")

        if self.extra_metadata is not None:
            query_clauses.append(f"{print_like_params('extra_metadata', self.extra_metadata)}")
    ...
    def _are_stimuli_valid(self, server: "AEPsychServer", config: Config, id: int) -> bool:
        pickeld_stimuli_per_trial = config.get("common", "stimuli_per_trial", fallback=-1)
        active_stimuli_per_trial = server.config.get("common", "stimuli_per_trial", fallback=0)

        if pickeld_stimuli_per_trial != active_stimuli_per_trial:
            print_warning(
                self,
                server.db.execute_sql_query(f"select * from master where {id} = unique_id", None),
                f"Data with master id: {id} was matched with {{0}} but is invalid due because it has " +\
    ...
    def _are_parameters_valid(self, server: "AEPsychServer", config: Config, id: int) -> bool:
    ...
    def _are_outcomes_valid(self, server: "AEPsychServer", config: Config, id: int) -> bool:
        pickled_outcome_types = config.getlist("common", "outcome_types", element_type=str)
        active_outcome_types = server.config.getlist("common", "outcome_types", element_type=str)
        ...
        if outcome_types_match:
            pickled_outcome_names = set(config.getlist("common", "outcome_names", element_type=str,
                fallback=generate_default_outcome_names(len(pickled_outcome_types))))

            active_outcome_names = set(server.config.getlist("common", "outcome_names", element_type=str,
                fallback=generate_default_outcome_names(len(active_outcome_types))))

            if len(pickled_outcome_names.intersection(active_outcome_names)) != len(active_outcome_names):
                print_warning(
                    self,
                    server.db.execute_sql_query(f"select * from master where {id} = unique_id", None),
                    f"Data with master id: {id} was matched with {{0}} but is invalid due to " +\
    ...
    def _is_ex_metadata_match_valid(self, config: Config) -> bool:
        pickled_ex_data = config.getMetadata(only_extra= True)
        if len(pickled_ex_data) == 0:
            return True
        
        potential_matches = pickled_ex_data.keys() & self.extra_matadata.keys()
        for key in potential_matches:
            if pickled_ex_data[key] == self.extra_metadata[key] or\
               (isinstance(self.extra_metadata[key], Iterable) and pickeld_ex_data[key] in self.extra_metadata[key]):
                return True
    ...
    def _get_valid_data_ids(self, server: "AEPsychServer") -> List[int]:
        ...
        Returns:
            List[int]: a list of master table ids that meed all criteria to be valid for use in the current strategy.
        """
        valid_match_ids = []
    ...
    def _get_data(self, server: "AEPsychServer", ids: List[int]) -> List[Tuple[Any]]:
        ...
        # !!!!!! WARNING !!!!!
        query = f"""select param_data.iteration_id, param_data.param_name,
                    ...
                    where master.unique_id in {print_in_params(ids)}"""

        results = server.db.execute_sql_query(query, None)
        return results if results is not None else []

    def warm_start_strat(self, server: "AEPsychServer", strat: Strategy):
        ...
        while i < len(data):
            ...
            config = { trim_param_name(data[i][PARAM_NAME_ID]): float(data[i][PARAM_VAL_ID]) }
            ...
            j = 1
            while i + j < len(data) and data[i][ITERATION_ID] == data[i + j][ITERATION_ID]:
                config[trim_param_name(data[i + j][PARAM_NAME_ID])] = float(data[i + j][PARAM_VAL_ID])
                j += 1
        ...
        logger.info(f"Strategy {strat.name} was warm started with {fetch_count} rows of data.")
    ...
    def get_config_options(
        ...
        if config.has_section(seed_conds_name):
            ...
            if "experiment_name" in seed_conds_data:
                experiment_names = config.getlist(seed_conds_name, "experiment_name", element_type=str)
                seed_conds_data.pop("experiment_name", None)

            if "experiment_id" in seed_conds_data:
                experiment_ids = config.getlist(seed_conds_name, "experiment_id", element_type=str)
                seed_conds_data.pop("experiment_id", None)

            if "experiment_description" in seed_conds_data:
                experiment_descriptions = config.getlist(seed_conds_name, "experiment_description", element_type=str)
                seed_conds_data.pop("experiment_description", None)

            if "participant_id" in seed_conds_data:
                participant_ids = config.getlist(seed_conds_name, "participant_id", element_type=str)
                seed_conds_data.pop("participant_id", None)

            extra_metadata = seed_conds_data if len(seed_conds_data) > 0 else None
        else:
            raise KeyError(f"config must have section {seed_conds_name} to warm start strategy.")
...
def print_warning(fetcher: DataFetcher, query_record: List[Tuple[Any]], format: str):
    first_match = True
    match_str = ""
    if fetcher.experiment_names and query_record[0][1] in fetcher.experiment_names:
        match_str += f"experiment_name = {query_record[0][1]}"
        first_match = False

    if fetcher.experiment_desc and query_record[0][2] in fetcher.experiment_desc:        
        if not first_match:
            match_str += " and "

        match_str += f"experimend_desc = {query_record[0][2]}"
        first_match = False

    if fetcher.experiment_ids and query_record[0][3] in fetcher.experiment_ids:
        if not first_match:
            match_str += " and "

        match_str += f"experimend_id = {query_record[0][3]}"
        first_match = False

    if fetcher.participant_ids and query_record[0][4] in fetcher.participant_ids:
        if not first_match:
            match_str += " and "

        match_str += f"participant_id = {query_record[0][4]}"
        first_match = False

    # assumes that all false positive metadata matches have been filtered out.
    if fetcher.extra_metadata and query_record[0][5]: 
        ex_data_str = "" if first_match else " and "
        ex_data = json.loads(query_record[0][5])

        first_ex_match = True
        for i, (key, val) in enumerate(fetcher.extra_metadata.items()):
            if key in ex_data and (val == ex_data[key] or (isinstance(val, Iterable) and ex_data[key] in val)):
