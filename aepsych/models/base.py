...
from .transformed_posteriors import MCTransformedPosterior
...
class AEPsychModelDeviceMixin(AEPsychMixin):
    ...
    def predict_transform(
        self,
        x: torch.Tensor,
        transformed_posterior: type[TransformedPosterior] = MCTransformedPosterior,
        **transform_kwargs,
    ):
        with torch.no_grad():
            x = x.to(self.device)
            post = self.posterior(x)
            post = transformed_posterior(post, **transform_kwargs)
