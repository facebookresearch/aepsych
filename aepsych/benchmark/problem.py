...
class Problem:
    ...
    def sample_y(
        ...
        np.ndarray
    ):  # TODO: This can be done with  torch.bernoulli(self.p(x)), but Strategy.add_data() expects a numpy array for now
    ...
    def evaluate(
        ...
        fsamps = model.sample(self.eval_grid, num_samples=1000).to(self.f_true)
        try:
            psamps = model.sample(
