    ...
    def setUpClass(cls):
        ...
        def make_prob_matrix(fgrid, cutpoints, n_levels):
            ...
            for i in range(1, n_levels - 1):
                probs[..., i] = norm.cdf(cutpoints[i] - fgrid) - norm.cdf(
                    cutpoints[i - 1] - fgrid
                )
        ...
        def simulate_trial(trial_config):
            ...
            cutpoints = np.quantile(
                novel_detection_testfun(x), np.linspace(0.05, 0.95, n_levels - 1)
            )
            p = make_prob_matrix(
                novel_detection_testfun(x), cutpoints=cutpoints, n_levels=n_levels
            )
            return (
                Categorical(probs=torch.Tensor(p))
                .sample(torch.Size([1]))
                .squeeze()
                .item()
            )
