...
class TransformsLog10Test(unittest.TestCase):
    def test_log_transform(self):
        ...
        self.assertTrue(torch.equal(transforms.untransform(transformed), values))
    ...
    def test_log10Plus_transform(self):
        ...
        values = torch.tensor([0, 0.5, 1])
        transformed = transforms.transform(values)
        untransformed = transforms.untransform(transformed)

        self.assertTrue(torch.all(transformed >= 0))
        self.assertTrue(torch.all(transformed == untransformed))

    def test_log10_neg_error(self):
        config_str = """
            [common]
            parnames = [signal1]
            stimuli_per_trial = 1
