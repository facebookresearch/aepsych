...
class DataFetcherTestCase(unittest.TestCase):
    ...
    def test_experiment_name_match(self):
        ...
        validation_set = set([
            1, 7, 9, 15, 18, 20, 27, 31, 32, 33, 34, 35, 36, 37, 38, 39, 
        ...
        logger.info(f"validation len: {len(validation_set)} ids len: {len(valid_ids)}")
        logger.info(f"{validation_set}")
        logger.info(f"{valid_ids}")
    ...
    def test_validate_data_query_constants(self):
        config_str = self.default_config() + self.pre_seed_config("seed_conds", exp_ids= ["multi_param_gen"])
        test_fetcher = DataFetcher.from_config(Config(config_str = config_str), "my_strat")

        validation_record = self.s.db.get_raw_for(1)[0]
        query_res = self.s.db.execute_sql_query(test_fetcher._construct_data_query([1]), None)[0]
        
        self.assertTrue(query_res[ITERATION_ID] == validation_record.children_param[0].iteration_id)
        self.assertTrue(query_res[PARAM_NAME_ID] == validation_record.children_param[0].param_name)
        self.assertTrue(query_res[PARAM_VAL_ID] == validation_record.children_param[0].param_value)
        self.assertTrue(query_res[OUTCOME_ID] == validation_record.children_outcome[0].outcome_value)
