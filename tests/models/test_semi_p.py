...
def _hadamard_model_constructor(lb, ub, stim_dim, floor, objective=FloorLogitObjective, inducing_point_method=AutoAllocator):
    return HadamardSemiPModel(
        ...
        inducing_size=10,
        inducing_point_method=AutoAllocator(bounds=torch.stack((torch.tensor(lb), torch.tensor(ub)))),
        max_fit_time=0.5,
    )


def _semip_model_constructor(lb, ub, stim_dim, floor, objective=FloorLogitObjective, inducing_point_method=AutoAllocator):
...
class SemiPSmokeTests(unittest.TestCase):
    ...
    def test_mc_generation(self, objective):
        ...
        model = SemiParametricGPModel(
            ...
            inducing_size=10,
            inducing_point_method=AutoAllocator(bounds=torch.stack((torch.tensor(self.lb), torch.tensor(self.ub)))),
        )
    ...
    def test_reset_variational_strategy(self, model_constructor):
        ...
        stim_dim = 0
        with self.subTest(model_constructor=model_constructor):
            model = model_constructor(lb=lb, ub=ub, stim_dim=stim_dim, floor=0, inducing_point_method=AutoAllocator(bounds=torch.stack((torch.tensor(lb), torch.tensor(ub)))))
...
class HadamardSemiPtest(unittest.TestCase):
    ...
    def test_reset_hyperparams(self):
        model = HadamardSemiPModel(lb=[-3, -3], ub=[3, 3], inducing_size=20, inducing_point_method=AutoAllocator(bounds=torch.stack((torch.tensor([-3, -3]), torch.tensor([3, 3])))))
